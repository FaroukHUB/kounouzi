...
- `questions/religion/oussoul-ath-thalatha.v1.json` — banque religieuse
  ...
  contrôle humain par `scripts/content/import-oussoul.mjs`. Importées `draft`,
  **validées humainement le 2026-09-04** (voir `validation.v1.json`) ; seules
  les cartes validées sont jouables. Le livre reste derrière le rideau :
...
- `questions/religion/wa-jaa-shahr-ramadan.v1.json` — banque « Wa Ja'a Shahr
  ...
  2014), 25 cartes en 5 niveaux, mêmes règles, validées humainement.
- `questions/religion/ad-durous-al-mouhimmah.v1.json` — banque « Ad-Durous
  ...
  `scripts/content/import-durous.mjs`, validées humainement. `animationHint` est la
  ...
  du choix A, tel quel. Un énoncé qui laisserait voir le texte ou le
  commentaire porte une `reviewNotes` à l'import : reformulé en relecture
  humaine (couche `corrections.v1.json`), jamais par le code.
- `questions/religion/sirah-al-urjuzah.v1.json` — banque « Sirah — al-Urjūzah
  ...
  DOCX de contrôle par le même script, validées humainement, arabe intact. Les
...
- `questions/religion/al-qawaid-al-arba.v1.json` — banque « Al-Qawāʿid
  ...
  5 niveaux, mêmes règles, validées humainement. Cinq explications arabes
  avaient perdu un signe diacritique à l'extraction : rétablies après
  vérification directe dans le PDF de contrôle (`corrections.v1.json`).
- `questions/religion/kalimah-at-tawhid.v1.json` — banque « Kalimah
  ...
  al-Badr), 25 cartes en 5 niveaux, mêmes règles, validées humainement, importées
  depuis le DOCX de contrôle (arabe intact). Les sources portent le thème du
  chapitre en `locator`.

- `questions/religion/corrections.v1.json` — corrections HUMAINES de la revue
  (réponses « lettre », énoncés « rideau », ponctuation, arabe vérifié
  caractère par caractère dans les PDF originaux, liste `sourceVerified`),
  réappliquées par `scripts/content/apply-corrections.mjs` à chaque réimport.
- `questions/religion/validation.v1.json` — **validation humaine** des six
  banques (375 identifiants, date, décision). Appliquée après les corrections
  par le même script : chaque carte listée passe à `validated` seulement si
  elle franchit la garde (FR + AR, source, aucune `reviewNotes`), sinon
  l'application échoue. Une carte non listée reste `draft` : un réimport ne
  fait jamais perdre une validation ni n'en crée une (ADR 0030).
