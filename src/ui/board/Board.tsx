...
export interface BoardProps {
  ...
  /** Couche des personnages (superposée) et contenu central. */
...
/**
 * Plateau Kounouzi en perspective : le moteur et la grille restent strictement
 * 2D/déterministes ; seule la couche visuelle ajoute profondeur et perspective.
 */
export function Board({ board, highlightedCell, arrivalCell, previewPath, pawns, center, holdings = [], sites = {}, players = [], profiles = [] }: BoardProps) {
  ...
  return (
    <div className="k-board-3d-stage relative w-full max-w-[min(94vw,80vh)]">
      <div
        className="k-board-3d-shell bg-wood relative w-full select-none rounded-[2.2rem] p-[2.2%]"
      ...
      >
        ...
        <div className="k-pawn-layer-3d pointer-events-none absolute inset-[3.6%]">{pawns}</div>
      </div>
    </div>
  );
}
