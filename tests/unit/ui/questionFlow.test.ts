...
describe("suite de la validation : explication réservée aux catégories qui la déclarent", () => {
  it("religion, géographie, logique et gestion affichent et lisent une explication après la réponse (donnée de configuration)", () => {
    ...
    expect(CATEGORIES.filter((c) => c.showsExplanation).map((c) => c.id)).toEqual(["religion", "geography", "logic", "management"]);
  });

  it("religion, géographie, logique et gestion : Correct / Presque / Incorrect mènent à l'explication", () => {
    for (const id of ["religion", "geography", "logic", "management"]) {
  ...
  it("maths et les autres : la réponse validée part directement au moteur, sans déclaration de maîtrise", () => {
    for (const id of ["maths", "history", "arabic", "culture"]) {
