...
describe("suite de la validation : explication réservée aux catégories qui la déclarent", () => {
  it("religion et géographie affichent et lisent une explication après la réponse (donnée de configuration)", () => {
    // Décision produit : dans ces deux catégories l'explication FAIT PARTIE de l'apprentissage —
    // plusieurs cartes de géographie portent leur nuance dans l'explication et la perdraient sans elle.
    expect(CATEGORIES.filter((c) => c.showsExplanation).map((c) => c.id)).toEqual(["religion", "geography"]);
  });

  it("religion et géographie : Correct / Presque / Incorrect mènent à l'explication", () => {
    for (const id of ["religion", "geography"]) {
  ...
  it("maths et les autres : la réponse validée part directement au moteur, sans déclaration de maîtrise", () => {
    for (const id of ["maths", "history", "arabic", "logic", "management", "culture"]) {
