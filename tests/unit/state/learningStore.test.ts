...
describe("learningStore (mémoire pédagogique persistante)", () => {
  ...
  it("un Défi famille dont la question validée a été jouée compte comme un essai (réussi = correct, raté = incorrect) ; sans question, rien", () => {
    ...
    const won: GameEvent[] = [{ type: "FamilyChallengeCompleted", playerId: pid("p1"), challengeId: "CH-099", success: true, requestId: "c1", question }, { type: "ChallengeRewardGranted", playerId: pid("p1"), challengeId: "CH-099", amount: 15 }];
    ...
    const lost: GameEvent[] = [{ type: "FamilyChallengeCompleted", playerId: pid("p1"), challengeId: "CH-099", success: false, requestId: "c2", question }];
